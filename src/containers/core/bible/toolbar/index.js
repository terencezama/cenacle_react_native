...
class BibleToolbar extends Component {
    ...
    //#region Actions
    _clearHighlight = () => {
        const { bible_underline } = this.props;
        let highlights = bible_underline.data.filter(element => {
        ...
        highlights = highlights.map(elem=>{
            return elem.verse
        })
        console.log('unhighlight',...highlights);
        this.props.unhighlightVersesIds([...highlights])
    ...
    render() {
        ...
        if (bible_underline.data.length !== 0) {
            ...
            const highlightRender = !isHighlighted ? null : (
                <TouchableOpacity style={[styles.iconContainer, { backgroundColor: "black" }]} onPress={() => this._clearHighlight()}>
...
const mapDispatchToProps = (dispatch) => ({
    ...
    unhighlightVersesIds: params => dispatch(performAction(params, Types.BIBLE_UNHIGHLIGHT_VERSES_IDS)),
